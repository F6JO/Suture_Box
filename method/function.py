...
def if_tool_ex(path,gongju,tool_list):
    panduan = True
    no_tool_list = []
        ...
        if gongju == 'all':
            for i in tool_list:
                if not os.path.exists(path + "/" + i):
                    panduan = False
                    no_tool_list.append(i)
        else:
            if not os.path.exists(path + "/" + gongju):
                panduan = False
                no_tool_list.append(gongju)
    else:
        no_tool_list = tool_list
        panduan = False
    return [panduan,no_tool_list]
